class Calculator {
    ...
    analyzeBtnClick(clickedBtn) {
      ...
      btns.forEach((btn) => {
        if (btn.textContent.trim() === clickedBtn.trim()) {
          btnType = btn;
          console.log(btn);
        }
      });
    ...
    activateBtn(type) {
      let btnType = type ? type.attributes[0].name.slice(5) : "";
  ...
  numberButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      calculator.analyzeBtnClick(button.innerText);
    });
  });
  ...
  equalsButton.addEventListener("click", (button) => {
    calculator.compute();
    calculator.updateDisplay();
    calculator.analyzeBtnClick(button.target.innerText);
  });
